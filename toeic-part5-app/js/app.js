...
// Trạng thái hệ thống
let currentTopicId = 'Cấu trúc câu';
...
function renderTestList(questions) {
  ...
  const numPerTest = TEST_SIZES[currentTopicId] || DEFAULT_TEST_SIZE;
  const numTests = Math.ceil(questions.length / numPerTest);
  
  for (let i = 0; i < numTests; i++) {
    const testQuestions = questions.slice(i * numPerTest, (i + 1) * numPerTest);
