...
function startTest(testQuestions, testTitle) {
  ...
  document.getElementById('test-header-title').textContent = testTitle || "TOEIC Grammar Test";
...
function askConfirmSubmit() {
  ...
  if (answeredCount < activeQuestions.length) {
    const cf = confirm(`Bạn mới trả lời ${answeredCount}/${activeQuestions.length} câu. Bạn có chắc chắn muốn nộp bài?`);
    if (!cf) return;
  } else {
    const cf = confirm(`Hoàn thành xuất sắc! Bấm OK để bắt đầu xem kết quả.`);
    if (!cf) return;
  }
  processSubmitTest();
}
